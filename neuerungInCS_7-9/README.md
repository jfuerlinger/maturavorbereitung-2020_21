...
Der `readonly`-Modifier kann auf Members der Strukturen angewendet werden. Es zeigt an, dass der Status des Members nicht geändert werden kann. Es ergeben sich damit granularere Möglichkeiten die Zugriffe zu beschränken.
Ein Beispiel für die Handhabung an folgender Struktur:
```cs
public struct Point
{
...
public readonly override string ToString() =>
    $"({X}, {Y}) is {Distance} from the origin";
```
Die vorhergehende Änderung generiert eine Compilerwarnung, weil ToString auf das `Distance`-Property zugreift, die nicht als readonly markiert ist:
```
...
```
Der `readonly`-Modifier ist bei einem schreibgeschützten Property erforderlich. Der Compiler geht nicht davon aus, das `get`-Zugriffsmethoden den Zustand nicht ändern. Eine Ausnahme bilden die `autogenerated Properties`. Der Compiler behandelt alle `autogenerated Properties` ohne `set` als `readonly`.

Der Compiler erzwingt die Regel, dass `readonly`-Member den Status nicht ändern. Die folgende Methode wird nicht kompiliert, es sei denn, man entfernt den readonly-Modifizierer:
```cs
...
```
Mit diesem Feature kann man die Designabsicht angeben, damit der Compiler basierend auf dieser Absicht die entsprechende Implementierung erzwingt.
